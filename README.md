...
filter_limits = { max_named_filters = 32, max_event_types = 4, max_name_bytes = 128, allow_all = true }
...
| account/transaction/entry filters | block, best-chain, finalized, and mempool event filters |
...
unary methods are intentionally not mirrored because Zakura has no equivalent
event or state model. Durable historical replay should remain a separate Kafka
or storage consumer so consensus and state tasks never wait for it.
